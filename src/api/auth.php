...
/**
 * Perfis pelo modelo novo (usuario_sistema -> perfil) + normalização (MASTER -> ADMIN_MASTER).
 */
...
/* ============================
   VALIDAR SENHA (seed fallback dev)
============================ */
$valid = false;

// Se sua senha_hash já for SHA2 (e não password_hash), password_verify vai falhar.
// Você está usando seed_password para destravar no dev — ok.
if (!empty($usuario['senha_hash']) && password_verify($senha, $usuario['senha_hash'])) {
    ...
    // converte seed -> password_hash e limpa seed_password
...
/* ============================
   BUSCAR PERFIS (FONTE DA VERDADE)
============================ */
$perfis = carregarPerfisUsuario($pdo, (int)$usuario['id_usuario']);

if (!$perfis) {
    http_response_code(403);
    echo json_encode(['message' => 'Usuário sem perfil autorizado']);
    exit;
}
