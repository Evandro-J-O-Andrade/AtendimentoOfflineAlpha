...
Objetivo: executor assistencial triagem iniciar conforme definida no dump SQL do sistema.
...
```sql
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_executor_assistencial_triagem_iniciar`(
