...
BEGIN
    ...
    DECLARE v_id_local_old BIGINT;

    DECLARE v_unidade_dest BIGINT;
    ...
    IF v_unidade_dest IS NULL THEN
        SET v_msg = 'Local destino inválido';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_msg;
    END IF;

    IF v_unidade_dest <> v_id_unidade THEN
        SET v_msg = 'Local destino não pertence à unidade da sessão';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_msg;
    END IF;

    SELECT status, id_sistema, id_unidade, id_local_operacional
      INTO v_status_old, v_id_sistema_senha, v_id_unidade_senha, v_id_local_old
      FROM senhas
     WHERE id = p_id_senha
     FOR UPDATE;
    ...
    IF v_status_old IN ('FINALIZADO','CANCELADO') THEN
        SET v_msg = CONCAT('Não é possível encaminhar no status atual: ', v_status_old);
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_msg;
    END IF;
    ...
    INSERT INTO auditoria_evento (
        ...
        CONCAT('Encaminhamento ', v_id_local_old, ' -> ', p_id_local_destino, '. Motivo=', COALESCE(p_motivo,'(vazio)')),
        NOW()
    );

    COMMIT;

    SELECT p_id_senha AS id, p_id_local_destino AS id_local_destino, 'AGUARDANDO' AS status;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
    ...
    IF v_status_old IN ('FINALIZADO','CANCELADO') THEN
        SET v_msg = CONCAT('Não é possível cancelar no status atual: ', v_status_old);
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_msg;
    END IF;
...
BEGIN
    ...
    IF v_status_old NOT IN ('EM_ATENDIMENTO','AGUARDANDO','EM_COMPLEMENTACAO') THEN
        SET v_msg = CONCAT('Não é possível finalizar a senha no status atual: ', v_status_old);
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_msg;
    END IF;

    IF v_id_paciente IS NULL OR v_id_ffa IS NULL THEN
        SET v_msg = 'ERRO: Não é possível finalizar uma senha sem vincular Paciente e FFA.';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_msg;
    END IF;
    ...
    INSERT INTO auditoria_evento (
        ...
        CONCAT('Finalização. status ', v_status_old, ' -> FINALIZADO (id_paciente=', v_id_paciente, ', id_ffa=', v_id_ffa, ')'),
        NOW()
    );
...
BEGIN
    ...
    IF v_status_old NOT IN ('CHAMANDO','AGUARDANDO','EM_COMPLEMENTACAO') THEN
        SET v_msg = CONCAT('Não é possível iniciar atendimento no status atual: ', v_status_old);
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_msg;
    END IF;
    ...
    INSERT INTO auditoria_evento (
        ...
        CONCAT('Início de atendimento. status ', v_status_old, ' -> EM_ATENDIMENTO (local=', v_id_local, ')'),
        NOW()
    );
...
BEGIN
    ...
    IF v_status_old <> 'CHAMANDO' THEN
        SET v_msg = CONCAT('Não compareceu só pode ocorrer quando status=CHAMANDO. Atual: ', v_status_old);
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = v_msg;
    END IF;
    ...
    INSERT INTO auditoria_evento (
        ...
        CONCAT('Marcado como não compareceu. status ', v_status_old, ' -> NAO_COMPARECEU (local=', v_id_local, ')'),
        NOW()
    );
...
-- Dump completed on 2026-02-01  5:59:45
