...
BEGIN
  ...
  BEGIN
    ...
    -- Log best-effort (fora da TX após rollback)
    IF v_id_sessao IS NOT NULL THEN
      ...
      VALUES
        (v_id_sessao, p_id_usuario, p_id_usuario, 'sessao_usuario', v_id_sessao, 'ERRO_SQL',
         CONCAT('SQLSTATE=',v_sqlstate,' errno=',v_errno,' msg=',LEFT(v_msg,200)),
         NOW());
    END IF;
  ...
  -- Encerra qualquer sessão ativa do usuário (um usuário = 1 sessão ativa por vez)
  ...
  INSERT INTO auditoria_evento
    (id_sessao_usuario, id_usuario, id_usuario_espelho, entidade, id_entidade, acao, detalhe, criado_em)
  VALUES
    (v_id_sessao, p_id_usuario, p_id_usuario, 'sessao_usuario', v_id_sessao, 'ABRIR',
...
-- Dump completed on 2026-02-01  5:37:57
