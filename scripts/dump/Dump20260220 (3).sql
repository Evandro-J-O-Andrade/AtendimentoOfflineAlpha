...
CREATE TABLE `estoque_local` (
  ...
  `codigo` varchar(60) COLLATE utf8mb4_general_ci NOT NULL,
  `tipo` enum('FARMACIA_RUA','FARMACIA_PA','FARMACIA_UPA','FARMACIA_UBS','ALMOX','LAB','OUTRO') COLLATE utf8mb4_general_ci NOT NULL,
  `ala` enum('ADULTO','PEDI') COLLATE utf8mb4_general_ci DEFAULT NULL,
...
CREATE TABLE `estoque_reserva` (
  ...
  `id_lote` bigint NOT NULL,
  `quantidade` decimal(14,3) NOT NULL,
  `origem_tipo` enum('FARM_DISP','PDV','AJUSTE','OUTRO') COLLATE utf8mb4_general_ci NOT NULL DEFAULT 'FARM_DISP',
  `id_documento` bigint DEFAULT NULL,
  `status` enum('ATIVA','CONFIRMADA','CANCELADA') COLLATE utf8mb4_general_ci NOT NULL DEFAULT 'ATIVA',
...
CREATE TABLE `estoque_reserva_evento` (
  ...
  `id_sessao_usuario` bigint NOT NULL,
  `tipo_evento` enum('CRIAR','CONFIRMAR','CANCELAR','ERRO') COLLATE utf8mb4_general_ci NOT NULL,
...
-- =====================================================
-- Stored Procedures: Dispensação Farmácia (Imutável)
-- Implementam: movimento append-only + reserva + dupla baixa
-- Sem triggers; usa auditoria_evento_registrar para rastreabilidade
-- =====================================================

-- SP: Registra dispensação (cria farm_dispensacao, cria estoque_movimento append-only, cria reserva se dupla-baixa)
DROP PROCEDURE IF EXISTS sp_farm_dispensacao_registrar;
DELIMITER $$
CREATE PROCEDURE sp_farm_dispensacao_registrar(
...
-- SP: Confirma reserva de estoque (dupla baixa, segunda assinatura)
DROP PROCEDURE IF EXISTS sp_farm_reserva_confirmar;
DELIMITER //

CREATE PROCEDURE sp_farm_reserva_confirmar(
    IN p_id_sessao_usuario BIGINT,
...
-- Fim dos SPs de Farmácia Imutável
...
-- Dump completed on 2026-02-21  0:45:58
