...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_fila_tipo_por_local`(
    IN  p_id_sessao_usuario     BIGINT,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_operacao_encaminhar`(
    IN p_id_sessao_usuario          BIGINT,
    IN p_id_ffa                     BIGINT,
    IN p_id_local_operacional_destino BIGINT -- NULL -> usa local da sessão (ex.: setor atual)
)
main: BEGIN
    ...
    -- Valida se o local destino existe
    CALL sp_assert_true(
        EXISTS(SELECT 1 FROM local_operacional lo WHERE lo.id_local_operacional = v_id_local_destino AND lo.ativo = 1),
        'NOT_FOUND',
        CONCAT('Local destino inexistente/inativo: ', v_id_local_destino)
    );

    START TRANSACTION;

    -- !!! FIX AQUI: ffa PK é `id`
    SELECT f.id_senha, s.lane, f.classificacao_cor
      INTO v_id_senha, v_lane, v_cor
      FROM ffa f
      JOIN senhas s ON s.id = f.id_senha
     WHERE f.id = p_id_ffa
     LIMIT 1
     FOR UPDATE;

    CALL sp_assert_true(v_id_senha IS NOT NULL, 'NOT_FOUND', CONCAT('FFA não encontrada: ', p_id_ffa));

    -- Upsert da fila operacional para o destino (1 FFA por local destino)
    SELECT fo.id_fila
      INTO v_id_fila
      FROM fila_operacional fo
     WHERE fo.id_ffa = p_id_ffa
       AND fo.id_local_operacional = v_id_local_destino
       AND fo.status IN ('AGUARDANDO','EM_EXECUCAO')
    ...
    IF v_id_fila IS NULL THEN
        INSERT INTO fila_operacional(
            id_ffa, id_senha, id_local_operacional,
            tipo, lane, cor, status,
            id_local, prioridade,
            criado_em, atualizado_em
        ) VALUES (
            p_id_ffa, v_id_senha, v_id_local_destino,
            'ATENDIMENTO', v_lane, v_cor, 'AGUARDANDO',
            NULL, 0,
            NOW(), NOW()
        );
        ...
        VALUES (v_id_fila, p_id_sessao_usuario, 'ENCAMINHAR', CONCAT('destino_local=', v_id_local_destino), NOW());
    ELSE
        ...
        INSERT INTO fila_operacional_evento(id_fila, id_sessao_usuario, tipo_evento, detalhe, criado_em)
        VALUES (v_id_fila, p_id_sessao_usuario, 'REENCAMINHAR', CONCAT('destino_local=', v_id_local_destino), NOW());
    END IF;
    ...
    CALL sp_auditoria_evento_registrar(
        ...
        p_id_ffa,
        'ENCAMINHAR',
        CONCAT('id_senha=', v_id_senha, ' | destino_local=', v_id_local_destino),
        NULL,
        'ffa',
        NULL
    );
...
-- Dump completed on 2026-02-15  7:19:35
