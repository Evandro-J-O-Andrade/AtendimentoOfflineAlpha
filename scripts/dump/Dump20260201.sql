...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_baixa_estoque_automatica` AFTER UPDATE ON `atendimento_checagem` FOR EACH ROW BEGIN
    -- Se o status mudou para 'REALIZADO'
    IF NEW.status = 'REALIZADO' AND OLD.status <> 'REALIZADO' THEN
        -- Aqui vinculamos o medicamento da prescrição ao produto do estoque
        -- (Num sistema real, usaríamos um ID de produto na prescrição)
        UPDATE estoque_produtos ep
        JOIN atendimento_prescricao ap ON ap.id = NEW.id_prescricao
        SET ep.quantidade_estoque = ep.quantidade_estoque - 1
        WHERE ep.descricao = ap.medicamento;
    END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `tr_internacao_valida_leito` BEFORE INSERT ON `internacao` FOR EACH ROW BEGIN
    DECLARE v_status_leito VARCHAR(50);

    -- Busca o status atual do leito
    SELECT status INTO v_status_leito FROM leito WHERE id_leito = NEW.id_leito;

    -- Se o leito não estiver disponível, trava a inserção
    IF v_status_leito <> 'DISPONIVEL' THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'ERRO: Este leito não está disponível para nova internação (Status atual: LIMPEZA/OCUPADO/MANUTENÇÃO).';
    END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `tr_fluxo_internacao_leito` AFTER INSERT ON `internacao` FOR EACH ROW BEGIN
    UPDATE leito SET status = 'OCUPADO' WHERE id_leito = NEW.id_leito;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `tr_internacao_ocupa_leito` AFTER INSERT ON `internacao` FOR EACH ROW BEGIN
    -- Atualiza o leito para OCUPADO automaticamente
    UPDATE leito SET status = 'OCUPADO' WHERE id_leito = NEW.id_leito;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `tr_alta_libera_limpeza` AFTER UPDATE ON `internacao` FOR EACH ROW BEGIN
    IF NEW.status = 'CONCLUIDA' AND OLD.status = 'ATIVA' THEN
        UPDATE leito SET status = 'LIMPEZA' WHERE id_leito = NEW.id_leito;
    END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `tr_internacao_alta_limpeza` AFTER UPDATE ON `internacao` FOR EACH ROW BEGIN
    -- Se o status da internação mudou para 'CONCLUIDA' (Alta)
    IF NEW.status = 'CONCLUIDA' AND OLD.status = 'ATIVA' THEN
        -- Libera o leito para a equipe de Higienização
        UPDATE leito SET status = 'LIMPEZA' WHERE id_leito = NEW.id_leito;
        
        -- Log de Hotelaria
        INSERT INTO log_auditoria (id_usuario, acao, tabela_afetada, id_registro, comentario)
        VALUES (1, 'ALTA_GIRO_LEITO', 'leito', NEW.id_leito, 'Leito enviado para limpeza automaticamente após alta.');
    END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `tr_validar_ocupacao_leito` BEFORE UPDATE ON `leito` FOR EACH ROW BEGIN
    -- Se alguém tentar mudar o status para OCUPADO sem ser via sistema de internação
    IF NEW.status = 'OCUPADO' AND OLD.status <> 'OCUPADO' THEN
        -- Verificamos se existe uma internação ativa para este leito
        IF NOT EXISTS (SELECT 1 FROM internacao WHERE id_leito = NEW.id_leito AND status = 'ATIVA') THEN
            SIGNAL SQLSTATE '45000' 
            SET MESSAGE_TEXT = 'ERRO: Não é possível ocupar um leito sem abrir uma Internação vinculada.';
        END IF;
    END IF;
END */;;
DELIMITER ;
...
INSERT INTO `senhas` VALUES (1,1,NULL,2,1,'A','A001','2026-01-28','CLINICO','ADULTO','RECEPCAO','AGUARDANDO',0,8,NULL,NULL,NULL,'2026-01-28 06:01:27',NULL,NULL,NULL);
...
BEGIN
    ...
    -- FIX: era UPDATE senha (tabela não existe). Correto: senhas.
    UPDATE senhas
    ...
    -- (opcional) aqui você decide se audita em auditoria_evento ou evento_ffa.
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_gerar_senha_v2` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_gerar_senha_v2`(
    IN p_id_sessao_totem BIGINT,
    IN p_prioridade TINYINT,
    IN p_tipo ENUM('CLINICO','PEDIATRICO','PRIORITARIO','EXAME')
)
BEGIN
    DECLARE v_id_unidade, v_id_sistema, v_id_local, v_id_usuario BIGINT;
    DECLARE v_prefixo CHAR(1);
    ...
    -- Contexto obrigatório via sessão
    SELECT id_unidade, id_sistema, id_local_operacional, id_usuario
      INTO v_id_unidade, v_id_sistema, v_id_local, v_id_usuario
      FROM sessao_usuario
     WHERE id_sessao_usuario = p_id_sessao_totem AND ativo = 1 LIMIT 1;

    IF v_id_unidade IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Sessao invalida ou sem contexto completo';
    END IF;

    SET v_prefixo = CASE 
        WHEN p_tipo = 'EXAME' THEN 'E'
        WHEN p_tipo = 'PRIORITARIO' OR p_prioridade > 0 THEN 'P'
        WHEN p_tipo = 'PEDIATRICO' THEN 'I'
        ELSE 'A'
    END;

    -- Sequência Transacional (Trava a linha para hospital grande)
    INSERT INTO senha_sequencia (id_unidade, id_sistema, prefixo, ultimo_numero, data_atual)
    VALUES (v_id_unidade, v_id_sistema, v_prefixo, 0, CURDATE())
    ON DUPLICATE KEY UPDATE ultimo_numero = ultimo_numero;

    SELECT ultimo_numero INTO v_proximo_num FROM senha_sequencia
     WHERE id_unidade = v_id_unidade AND id_sistema = v_id_sistema AND prefixo = v_prefixo AND data_atual = CURDATE() FOR UPDATE;

    SET v_proximo_num = v_proximo_num + 1;
    
    UPDATE senha_sequencia SET ultimo_numero = v_proximo_num 
     WHERE id_unidade = v_id_unidade AND id_sistema = v_id_sistema AND prefixo = v_prefixo AND data_atual = CURDATE();

    SET v_codigo = CONCAT(v_prefixo, LPAD(v_proximo_num, 3, '0'));

    -- Inserção da Senha
    INSERT INTO senhas (codigo, data_ref, id_unidade, id_sistema, id_local_operacional, prioridade, status, origem, tipo, id_usuario_operador)
    VALUES (v_codigo, CURDATE(), v_id_unidade, v_id_sistema, v_id_local, p_prioridade, 'GERADA', 'TOTEM', p_tipo, v_id_usuario);

    SET v_id_senha = LAST_INSERT_ID();

    -- Auditoria
    INSERT INTO auditoria_evento (id_sessao_usuario, id_usuario_espelho, entidade, id_entidade, acao, detalhe, criado_em)
    VALUES (p_id_sessao_totem, v_id_usuario, 'senhas', v_id_senha, 'GERACAO', CONCAT('Senha ', v_codigo, ' gerada'), NOW());

    COMMIT;
    SELECT v_id_senha AS id, v_codigo AS codigo;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2026-02-01  0:37:08
