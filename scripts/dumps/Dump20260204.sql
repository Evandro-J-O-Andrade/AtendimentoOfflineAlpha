...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_totem_satisfacao_resumo_hoje` AS select cast(`tf`.`data_hora` as date) AS `dia`,`tf`.`origem` AS `painel_tipo`,`tf`.`nota` AS `avaliacao`,count(0) AS `qtd` from `totem_feedback` `tf` where (cast(`tf`.`data_hora` as date) = curdate()) group by cast(`tf`.`data_hora` as date),`tf`.`origem`,`tf`.`nota` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-02-04  2:21:43
