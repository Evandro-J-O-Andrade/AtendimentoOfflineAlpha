...
authRouter.post('/login', async (req, res) => {
  const { login, token_jwt, refresh_token, ip, device } = req.body ?? {}
  ...
  try {
    const response: AuthLoginResponse = await authService.login(login, token_jwt, refresh_token ?? '', ip ?? '', device ?? '')
